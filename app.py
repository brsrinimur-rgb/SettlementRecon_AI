...
st.caption('Bank & POS Settlement Reconciliation • Monthly Finance Control  •  v1.1')
...
with st.sidebar:
    ...
    max_shift=st.number_input('Maximum settlement date shift (days)',0,MAX_ALLOWED_DATE_SHIFT,DEFAULT_MAX_DATE_SHIFT,1,
                               help='ANB settlement can lag several days. A match found via date shift is always '
                                    'labelled "Late Settlement / Date Shift Match", never silently shown as a plain Matched.')
    st.divider()
    ...
    with st.spinner('Processing monthly files...'):
        ...
        if terminal_file:
            ...
            try:
                ...
                st.warning(f'Saved default master could not be used: {e}')
                tm=pd.DataFrame()
            except Exception:
                tm=pd.DataFrame()

        pos_clean,pagg,bank_sett,recon,mapping_review=reconcile(pos,bank,tm,tolerance,float(max_shift))
        st.session_state['run']=(pos_clean,pagg,bank_sett,recon,mapping_review,bank_audit,tm)

if 'run' in st.session_state:
    pos_clean,pagg,bank_sett,recon,mapping_review,bank_audit,tm=st.session_state['run']
    total_pos=recon.pos_gross.sum(); total_bank=recon.gross_credit.sum(); diff=total_pos-total_bank
    matched=recon.status.isin(MATCHED_STATUSES).sum(); total=len(recon); match_pct=(matched/total*100 if total else 0)
    ...
    tabs=st.tabs(['Dashboard','Detailed Recon','Date-wise','Store-wise','Date + Store','Exceptions',
                  'Mapping Review','Parser Audit','Raw Control'])
    with tabs[0]:
        ...
        scheme_totals=recon.groupby('scheme_group',as_index=False).agg(POS_Gross=('pos_gross','sum'),Bank_Gross=('gross_credit','sum'),Records=('status','size'))
    ...
    with tabs[5]: st.dataframe(recon[~recon.status.isin(MATCHED_STATUSES)],use_container_width=True,hide_index=True)
    with tabs[6]:
        st.subheader('Terminals needing a store mapping')
        st.caption('These terminal IDs appeared in the bank statement or POS files but are not in the Terminal ID '
                   'Master. No store name has been guessed — add them to the master once confirmed.')
        if mapping_review:
    ...
    def make_excel():
        ...
        with pd.ExcelWriter(bio,engine='openpyxl') as w:
            summary=pd.DataFrame({'Metric':['Company','Month','POS Gross','Bank Gross','Difference','Matched / Late Settlement','Total Recon Rows','Match %','Terminals Needing Mapping','Dropped Bank Lines'],
                                  'Value':[company,month,total_pos,total_bank,diff,matched,total,match_pct,len(mapping_review),0 if bank_audit is None else len(bank_audit)]})
            ...
            recon[~recon.status.isin(MATCHED_STATUSES)].to_excel(w,'Exceptions',index=False)
